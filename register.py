...
def registerManually(caller, table):
    ...
    tableCols = si.fetchData(caller.sqlConnection, f"SHOW COLUMNS FROM `{table}`;")
    ...
    tableRefs = si.fetchData(
        caller.sqlConnection
      , si.buildQueryString("./sql/GET_TABLE_REFS.SQL", {"table": table})
    )

    # Get primary key and current max.
    primaryKey = si.getPrimaryKey(caller.sqlConnection, table)
    currentMax = si.fetchData(
        caller.sqlConnection, f"SELECT MAX({primaryKey}) AS currentMax FROM `{table}`"
    )[0]["currentMax"]

    # Calculate new max.
    try:
        newMax = n.getNextNumber(currentMax)
    except Exception as e:
    ...
    for l in nonRefs:
        # Some fields aren't foreign or primary keys but still need to
        # be generated by system logic.
        try:
            import importlib  # Load a module when it's provided as a string.

            # A module can be set in the config.
            loadMod = importlib.import_module(caller.config["tables"][table][l]["get by"]["module"])
    ...
    for ref in tableRefs:
        ...
        # Problems which can arise.
            ...
            print("Error: Important field could not be retrieved.")
            return -1
        except Exception as e:
        ...
        import sql_helpers as sh  # Obtain options.
        options = sh.getOptions(caller.sqlConnection, refToTable)
